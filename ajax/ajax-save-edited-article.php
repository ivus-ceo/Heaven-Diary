<?php
  ...
  if ($check_article_id != $article_id) {
    ...
    if ($articles_output['article_type'] == 1) {

      $article_content = $_POST['article_content'];

      if (!empty($article_content) && !empty($article_id)) {
        $sql = "UPDATE articles SET article_content = ? WHERE article_id = ?";
    ...
    } elseif ($articles_output['article_type'] == 2) {
      ...
      if (!empty($article_content) && !empty($article_id)) {
        $sql = "UPDATE articles SET article_content = ?, article_title = ?, article_image = ?, article_visible = ? WHERE article_id = ?";
        $stmt = mysqli_stmt_init($link);
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          header('Location: ../profile.php?error=sql-error');
          exit();
        } else {
          mysqli_stmt_bind_param($stmt, "sssii", $article_content, $article_title, $article_image, $article_visible, $article_id);
          mysqli_stmt_execute($stmt);
        }
      }
    }
