<?php
  ...
  $article_time = time();
  $article_data = array();
  parse_str($data, $article_data);

  $article_uid = $article_data['user_id'];
  $article_type = $article_data['type'];
  $article_visible = $article_data['visible'];
  $article_author = $article_data['author'];
  $article_author_image = $article_data['user_image'];

  $author = $_SESSION['name']." ".$_SESSION['last'];

  if ($article_uid != $_SESSION['id'] || $article_author != $author || $article_author_image != $_SESSION['image']) {
  ...
  if ($article_type == 2) {
    ...
    $sql = "INSERT INTO articles (article_uid, article_type, article_visible, article_author, article_author_image, article_image, article_title, article_date, article_time, article_content) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_stmt_init($link);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      header('Location: ../Profile.php?error=sql-error');
      exit();
    } else {
      mysqli_stmt_bind_param($stmt, "iiisssssss", $article_uid, $article_type, $article_visible, $article_author, $article_author_image, $article_image, $article_title, $article_date, $article_time, $article_content);
      mysqli_stmt_execute($stmt);
    }

  } elseif ($article_type == 3) {

    $article_title = $article_data['survey_title'];

    $sql = "INSERT INTO articles (article_uid, article_type, article_visible, article_author, article_author_image, article_title, article_date, article_time, article_content) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_stmt_init($link);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      header('Location: ../Profile.php?error=sql-error');
      exit();
    } else {
      mysqli_stmt_bind_param($stmt, "iiissssss", $article_uid, $article_type, $article_visible, $article_author, $article_author_image, $article_title, $article_date, $article_time, $article_content);
      mysqli_stmt_execute($stmt);
    }

  } else {

    $sql = "INSERT INTO articles (article_uid, article_type, article_visible, article_author, article_author_image, article_date, article_time, article_content) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_stmt_init($link);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      header('Location: ../Profile.php?error=sql-error');
      exit();
    } else {
      mysqli_stmt_bind_param($stmt, "iiisssss", $article_uid, $article_type, $article_visible, $article_author, $article_author_image, $article_date, $article_time, $article_content);
      mysqli_stmt_execute($stmt);
    }
